...
async def run_latency_audit():
    ...
    df = pd.read_parquet("scratch/load_test_100k.parquet").head(1)
    txn_data = df.to_dict('records')[0]
    ...
    for _ in range(100): 
        try:
            ...
            pass
    measurements.append(("database_persistence", (time.perf_counter() - t0) / 100))
